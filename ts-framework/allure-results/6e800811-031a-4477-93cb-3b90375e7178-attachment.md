...
```ts
  ...
  29  | 
  30  |   // Initialize Components in requested order
  ...
  57  |   await page.waitForTimeout(5000); // Wait for potential new tab and redirection
  58  |   
  59  |   const pages = page.context().pages();
  60  |   const trackerPageObj = pages.find((p) => p.url().includes('app-tracker')) || page;
  61  |   
  62  |   console.log(`Current Tracker URL: ${trackerPageObj.url()}`);
  63  |   
  64  |   // Update components to use the new page object in requested order
  65  |   const effectiveTopNav = new TopNavigationControls(trackerPageObj);
  66  |   const effectiveFilterChips = new ActiveFilterChips(trackerPageObj);
  67  |   const effectiveSearchBar = new FilterSearchBar(trackerPageObj);
  68  |   const effectivePagination = new PaginationFooter(trackerPageObj);
  69  |   const effectiveTable = new PolicyListTable(trackerPageObj);
  70  |   const effectiveDetails = new DetailDrawer(trackerPageObj);
  71  | 
> 72  |   await trackerPageObj.waitForLoadState('networkidle', { timeout: 15000 });
      |                        ^ TimeoutError: page.waitForLoadState: Timeout 15000ms exceeded.
  73  |   
  74  |   // 1. Top Navigation Validation (Requested First)
  75  |   console.log('Validating Top Navigation...');
  76  |   const topNavStatus = await effectiveTopNav.validateAll();
  77  |   console.log(`Top Nav Title: ${topNavStatus.pageTitle}`);
  78  |   expect(topNavStatus.logoVisible, 'Logo should be visible').toBeTruthy();
  79  |   expect(topNavStatus.pageTitle?.toLowerCase()).toContain('tracker');
  80  | 
  81  |   // 2. Filter Chips Validation
  82  |   console.log('Validating Filter Chips...');
  83  |   const chipStatus = await effectiveFilterChips.validateAll();
  84  |   expect(chipStatus.visible).toBeTruthy();
  85  | 
  86  |   // 3. Search Bar Validation
  87  |   console.log('Validating Search Bar...');
  88  |   const searchBarStatus = await effectiveSearchBar.validateAll();
  89  |   expect(searchBarStatus.searchBarVisible).toBeTruthy();
  90  | 
  91  |   // 4. Pagination Validation
  92  |   console.log('Validating Pagination...');
  93  |   const paginationStatus = await effectivePagination.validateAll();
  94  |   if (paginationStatus.visible) {
  95  |     console.log('Pagination is visible and validated.');
  96  |   } else {
  97  |     console.log('Pagination is not visible (expected for single page results).');
  98  |   }
  99  | 
  100 |   // 5. Policy Table Validation
  101 |   console.log('Validating Policy Table...');
  102 |   const tableStatus = await effectiveTable.validateAll();
  103 |   expect(tableStatus.visible).toBeTruthy();
  104 |   expect(tableStatus.rowCount).toBeGreaterThan(0);
  105 | 
  106 |   // 6. Search Interaction Validation
  107 |   if (tableStatus.rowCount > 0) {
  108 |     const firstAppNo = tableStatus.firstRow.app_no;
  109 |     console.log(`Searching for Application No: ${firstAppNo}`);
  110 |     await effectiveSearchBar.search(firstAppNo);
  111 |     await trackerPageObj.waitForTimeout(3000); // Wait for results to filter
  112 |     
  113 |     const filteredCount = await effectiveTable.getRowCount();
  114 |     console.log(`Filtered row count: ${filteredCount}`);
  115 |     expect(filteredCount).toBeGreaterThan(0);
  116 |   }
  117 | 
  118 |   // 7. Detail Drawer Validation (Requested Last)
  119 |   if (tableStatus.rowCount > 0) {
  120 |     console.log('Validating Detail Drawer...');
  121 |     // Click the first row to open the drawer
  122 |     await effectiveTable.clickRow(0);
  123 |     
  124 |     console.log('Waiting for Drawer to open...');
  125 |     const drawerOpened = await effectiveDetails.waitUntilOpen();
  126 |     expect(drawerOpened, 'Detail Drawer should open after clicking a row').toBeTruthy();
  127 |     
  128 |     const drawerStatus = await effectiveDetails.validateAll();
  129 |     console.log(`Drawer header name: ${drawerStatus.headerName}`);
  130 |     expect(drawerStatus.headerName).toBeTruthy();
  131 |     
  132 |     // Close the drawer
  133 |     console.log('Closing Detail Drawer...');
  134 |     await effectiveDetails.close();
  135 |     await trackerPageObj.waitForTimeout(1000);
  136 |     expect(await effectiveDetails.isOpen()).toBeFalsy();
  137 |     console.log('Drawer closed successfully.');
  138 |   }
  139 | 
  140 |   const pageUrl = trackerPageObj.url();
  141 |   expect(pageUrl).toContain('app-tracker');
  142 | });
  143 | 
  144 | function defaultPaths() {
  145 |   const envPaths = [
  146 |     path.resolve(__dirname, '..', '..', '..', 'app-tracker-automation', '.env'),
  147 |     path.resolve(__dirname, '..', '..', '..', '.env'),
  148 |     path.resolve(__dirname, '..', '..', '..', 'app-tracker-automation', 'tests', '.env')
  149 |   ];
  150 | 
  151 |   for (const envPath of envPaths) {
  152 |     if (fs.existsSync(envPath)) {
  153 |       dotenv.config({ path: envPath });
  154 |       break;
  155 |     }
  156 |   }
  157 | }
