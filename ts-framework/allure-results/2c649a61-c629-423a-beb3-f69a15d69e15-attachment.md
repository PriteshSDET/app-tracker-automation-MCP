...
```ts
  13  | import { PolicyListTable } from '../../components/policy_list_table';
  14  | import { TopNavigationControls } from '../../components/top_navigation_controls';
  ...
  31  |   const filterChips = new ActiveFilterChips(page);
  32  |   const searchBar = new FilterSearchBar(page);
  33  |   const pagination = new PaginationFooter(page);
  34  |   const policyTable = new PolicyListTable(page);
  35  |   const topNav = new TopNavigationControls(page);
  ...
  47  |   await menuBtn.waitFor({ state: 'visible', timeout: 5000 });
  48  |   await menuBtn.click();
  49  | 
  50  |   await page.waitForTimeout(5000); // Wait for menu to settle
  51  | 
  52  |   const trackerMenuItem = locators.applicationTrackerItem.first();
  53  |   await trackerMenuItem.waitFor({ state: 'visible', timeout: 5000 });
  54  |   await trackerMenuItem.click({ force: true });
  55  | 
  56  |   await page.waitForTimeout(2000);
  57  |   const pages = page.context().pages();
  58  |   const trackerPageObj = pages.find((p) => p.url().includes('app-tracker')) || page;
  59  |   
  60  |   // Update components to use the new page object if a new tab was opened
  61  |   const effectiveFilterChips = trackerPageObj !== page ? new ActiveFilterChips(trackerPageObj) : filterChips;
  62  |   const effectiveSearchBar = trackerPageObj !== page ? new FilterSearchBar(trackerPageObj) : searchBar;
  63  |   const effectivePagination = trackerPageObj !== page ? new PaginationFooter(trackerPageObj) : pagination;
  64  |   const effectiveTable = trackerPageObj !== page ? new PolicyListTable(trackerPageObj) : policyTable;
  65  |   const effectiveTopNav = trackerPageObj !== page ? new TopNavigationControls(trackerPageObj) : topNav;
  66  |   const effectiveDetails = trackerPageObj !== page ? new DetailDrawer(trackerPageObj) : details;
  67  | 
  68  |   await trackerPageObj.waitForLoadState('networkidle', { timeout: 10000 });
  69  |   
  70  |   // 1. Top Navigation Validation
  71  |   console.log('Validating Top Navigation...');
  72  |   const topNavStatus = await effectiveTopNav.validateAll();
  73  |   expect(topNavStatus.logoVisible).toBeTruthy();
  74  |   expect(topNavStatus.pageTitle).toBe('App Tracker');
  75  | 
  76  |   // 2. Filter Chips Validation
  77  |   console.log('Validating Filter Chips...');
  78  |   const chipStatus = await effectiveFilterChips.validateAll();
  79  |   expect(chipStatus.visible).toBeTruthy();
  80  | 
  81  |   // 3. Search Bar Validation
  82  |   console.log('Validating Search Bar...');
  83  |   const searchBarStatus = await effectiveSearchBar.validateAll();
  84  |   expect(searchBarStatus.searchBarVisible).toBeTruthy();
  85  | 
  86  |   // 4. Policy Table Validation
  87  |   console.log('Validating Policy Table...');
  88  |   const tableStatus = await effectiveTable.validateAll();
  89  |   expect(tableStatus.visible).toBeTruthy();
  90  |   expect(tableStatus.rowCount).toBeGreaterThan(0);
  91  | 
  92  |   // 5. Pagination Validation
  93  |   console.log('Validating Pagination...');
  94  |   const paginationStatus = await effectivePagination.validateAll();
  95  |   if (paginationStatus.visible) {
  96  |     console.log('Pagination is visible and validated.');
  97  |   } else {
  98  |     console.log('Pagination is not visible (likely single page results).');
  99  |   }
  100 | 
  101 |   // 6. Detail Drawer Validation
  102 |   if (tableStatus.rowCount > 0) {
  103 |     console.log('Validating Detail Drawer...');
  104 |     // Click the first row to open the drawer
  105 |     await effectiveTable.clickRow(0);
  106 |     
  107 |     const drawerStatus = await effectiveDetails.validateAll();
  108 |     expect(drawerStatus.isOpen).toBeTruthy();
  109 |     console.log(`Drawer opened for applicant: ${drawerStatus.headerName}`);
  110 |     
  111 |     // Close the drawer
  112 |     await effectiveDetails.close();
> 113 |     expect(await effectiveDetails.isOpen()).toBeFalsy();
      |                                             ^ Error: expect(received).toBeFalsy()
  114 |     console.log('Drawer closed successfully.');
  115 |   }
  116 | 
  117 |   // 7. Search Interaction Validation
  118 |   if (tableStatus.rowCount > 0) {
  119 |     const firstAppNo = tableStatus.firstRow.app_no;
  120 |     console.log(`Searching for Application No: ${firstAppNo}`);
  121 |     await effectiveSearchBar.search(firstAppNo);
  122 |     await trackerPageObj.waitForTimeout(2000); // Wait for results to filter
  123 |     
  124 |     const filteredCount = await effectiveTable.getRowCount();
  125 |     console.log(`Filtered row count: ${filteredCount}`);
  126 |     expect(filteredCount).toBeGreaterThan(0);
  127 |   }
  128 | 
  129 |   const pageUrl = trackerPageObj.url();
  130 |   expect(pageUrl).toContain('app-tracker');
  131 | });
  132 | 
  133 | function defaultPaths() {
  134 |   const envPaths = [
  135 |     path.resolve(__dirname, '..', '..', '..', 'app-tracker-automation', '.env'),
  136 |     path.resolve(__dirname, '..', '..', '..', '.env'),
  137 |     path.resolve(__dirname, '..', '..', '..', 'app-tracker-automation', 'tests', '.env')
  138 |   ];
  139 | 
  140 |   for (const envPath of envPaths) {
  141 |     if (fs.existsSync(envPath)) {
  142 |       dotenv.config({ path: envPath });
  143 |       break;
  144 |     }
  145 |   }
  146 | }
