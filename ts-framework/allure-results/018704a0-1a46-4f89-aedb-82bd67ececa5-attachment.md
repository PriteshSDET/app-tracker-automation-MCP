        ...
        - generic [ref=e51]:
          - generic [ref=e53] [cursor=pointer]:
            - generic [ref=e54]:
              ...
              - generic [ref=e56]:
                - generic [ref=e57]: ABSLI Poorna Suraksha Kawach | INDIAN BANK LIMITED (IB)
                - generic [ref=e59]:
                  - generic [ref=e60]: Application Status
                  - generic [ref=e61]: "|"
...
```ts
  ...
  28 | 
  29 |   const menuBtn = locators.menuButton.first();
  30 |   await menuBtn.scrollIntoViewIfNeeded();
  31 |   await menuBtn.waitFor({ state: 'visible', timeout: 5000 });
  32 |   await menuBtn.click();
  33 | 
  34 |   await page.waitForTimeout(500);
  35 | 
  36 |   const trackerMenuItem = locators.applicationTrackerItem.first();
  37 |   await trackerMenuItem.waitFor({ state: 'visible', timeout: 5000 });
  38 |   await trackerMenuItem.click({ force: true });
  39 | 
  40 |   await page.waitForTimeout(2000);
  41 |   const pages = page.context().pages();
  42 |   const trackerPageObj = pages.find((p) => p.url().includes('app-tracker')) || page;
  43 |   const effectiveTrackerPage = trackerPageObj !== page ? new AdityaBirlaTrackerPage(trackerPageObj) : trackerPage;
  44 | 
  45 |   await trackerPageObj.waitForLoadState('networkidle', { timeout: 10000 });
  46 |   await effectiveTrackerPage.waitForPageLoad(15000);
  47 | 
> 48 |   expect(await effectiveTrackerPage.isTrackerTableVisible()).toBeTruthy();
     |                                                              ^ Error: expect(received).toBeTruthy()
  49 |   expect(await effectiveTrackerPage.isApplicationSearchVisible()).toBeTruthy();
  50 | 
  51 |   const pageUrl = trackerPageObj.url();
  52 |   expect(pageUrl).toContain('app-tracker');
  53 | });
  54 | 
  55 | function defaultPaths() {
  56 |   const envPaths = [
  57 |     path.resolve(__dirname, '..', '..', '..', 'app-tracker-automation', '.env'),
  58 |     path.resolve(__dirname, '..', '..', '..', '.env'),
  59 |     path.resolve(__dirname, '..', '..', '..', 'app-tracker-automation', 'tests', '.env')
  60 |   ];
  61 | 
  62 |   for (const envPath of envPaths) {
  63 |     if (fs.existsSync(envPath)) {
  64 |       dotenv.config({ path: envPath });
  65 |       break;
  66 |     }
  67 |   }
  68 | }
