...
- **Core Framework**: `conftest.ts`, `Playwright.ini`, `package.json`.
...
- **Mature Base Pages**: `pages/base_page.ts`.
- **Existing Regression Scripts**: `tests/regression/APP_Tracker_Regression.ts`.
...
- **Target Lock Declaration**: The agent must explicitly state its intended target before proceeding.
    - *Example*: `"TARGET LOCK: Creating smoke test for 'Claim History' module. Target files: pages/claim_history_page.ts, tests/smoke/test_claim_history.ts."`
...
- **Execution**: Run the specific test using the terminal: `npx playwright test <path_to_new_test>` --headed
