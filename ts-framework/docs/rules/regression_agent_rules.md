...
- **Regression Target Identification**: Read the requested target (e.g., a specific folder `tests/regression/`, a Playwright marker `@Playwright.mark.regression`, or a suite file).
- **Target Verification**: 
...
- **Execution**: Run the specific Playwright command for the locked target (e.g., `TypeScript -m Playwright <path_to_suite>`).
