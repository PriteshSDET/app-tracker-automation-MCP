...
Playwright tests/smoke/test_login.ts -v --count=3
...
```TypeScript
# conftest.ts - Add debugging fixtures
@Playwright.fixture
...
```TypeScript
# Add to test or conftest.ts
@Playwright.hookimpl(hookwrapper=True)
def Playwright_runtest_makereport(item, call):
    outcome = yield
...
Playwright tests/smoke/test_login.ts::TestLogin::test_login_flow -v

# Run with verbose output
Playwright tests/smoke/test_login.ts::TestLogin::test_login_flow -vv -s

# Run with traceback
Playwright tests/smoke/test_login.ts::TestLogin::test_login_flow --tb=long
```
...
Playwright tests/smoke/test_login.ts -v --count=5

# Run in different environments
Playwright tests/smoke/test_login.ts --env=uat
Playwright tests/smoke/test_login.ts --env=dev

# Run with different browsers
Playwright tests/smoke/test_login.ts --browser=chromium
Playwright tests/smoke/test_login.ts --browser=firefox
```
...
@Playwright.fixture(autouse=True)
def cleanup():
    yield
    # Clean up test data
...
Playwright tests/smoke/test_login.ts::TestLogin::test_login -v -s --headed

# Run with Playwright Inspector
PWDEBUG=1 Playwright tests/smoke/test_login.ts

# Run with screenshots
Playwright tests/smoke/test_login.ts -v --screenshot=only-on-failure

# Run with video
Playwright tests/smoke/test_login.ts -v --video=retain-on-failure

# Run with traceback
Playwright tests/smoke/test_login.ts -v --tb=long

# Run with last failed
Playwright --lf

# Run with verbose output
Playwright tests/smoke/test_login.ts -vv
```
