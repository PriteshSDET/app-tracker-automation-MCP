...
- [ ] **Environment Variables**
  ...
  TypeScript -c "import os; from dotenv import load_dotenv; load_dotenv(); print(os.getenv('BASE_URL'))"
  ```
...
- [ ] **Import Check**
  ```bash
  TypeScript -m py_compile tests/smoke/test_login.ts
  ```

- [ ] **Syntax Check**
  ```bash
  Playwright --collect-only tests/smoke/
  ```

#### 3. Configuration Check
- [ ] **conftest.ts Configuration**
...
- [ ] **TypeScriptPATH Configuration**
  ```bash
  export TypeScriptPATH="${TypeScriptPATH}:$(pwd)"
  # Or in .env
  TypeScriptPATH=/path/to/project
  ```
...
# Quick config check
TypeScript -c "import os; from dotenv import load_dotenv; load_dotenv(); print('BASE_URL:', os.getenv('BASE_URL'))"

# Quick test run (smoke)
Playwright tests/smoke/ -v --maxfail=1
```
...
**Usage:**
```bash
TypeScript pre_flight_check.ts
Playwright tests/smoke/  # Only if checks pass
```
